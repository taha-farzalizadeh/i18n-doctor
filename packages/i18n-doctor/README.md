...
- Shared catalog modules: usage in any sibling namespace on the same source file clears unused for all

### What's new in 0.11.14

- Route tab titles via `getRouteParam(..., "title")`; nested route `settings`/`children`
- `for…in` `t(key)` over string-enum API maps without false missing keys
- Spread catalog keys underline the origin module; static `"a" + "b"` catalog values

### What's new in 0.11.13

- Live unused-key updates and incremental locale catalog refresh
- Broader static usages: factory configs, `useMemo` menus, remapped labels, `FILTER_OPERATOR_*` templates
- ESLint stale-range + unsaved locale overlay fixes
...
This is v0.11.17 — a beta release. Core analysis works on real projects but edge cases exist.
