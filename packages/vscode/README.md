...
- Fix Windows false unused/missing when distinct files were dropped as fake hardlinks (`bigint` inodes + size/mtime guard)
...
- Shared catalog modules: a key used in any namespace that shares the same source file clears unused for all of them (e.g. settings keys spread into map)
- **Unified versioning:** npm packages, language-server, and IDE plugins are all **0.11.15**
- Bundled language server @ 0.11.15
...
- Deleting an unused catalog key keeps remaining unused underlines (no ~15s gap)
- Incremental JSON/YAML locale refresh; key-range guards against wrong underlines
- Factory configs, `useMemo` menus, remapped operators, prefixed `FILTER_OPERATOR_*` templates
- **Unified versioning:** npm packages, language-server, and IDE plugins are all **0.11.13**
- Bundled language server @ 0.11.13
...
- **0.11.17** — Windows file-id hardlink false positives
